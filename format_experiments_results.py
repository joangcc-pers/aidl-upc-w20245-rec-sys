...
def generate_markdown_report(csv_file, md_file):
    ...
    sorted_by_recall = sorted(last_epoch.values(), key=lambda x: float(x['metrics'].get('Validation/recall@20', 0)), reverse=True)#[:5]
    sorted_by_mrr = sorted(last_epoch.values(), key=lambda x: float(x['metrics'].get('Validation/mrr@20', 0)), reverse=True)#[:5]
    ...
    with open(md_file, 'w') as f:
        ...
        f.write("| weight_decay | dropout_rate | learning_rate | epoch | R@20 | MRR@20 | Loss |\n")
        f.write("|-------------|--------------|---------------|-------|------|--------|------|\n")
        for row in sorted_by_recall:
            f.write(f"| {row['weight_decay']} | {row['dropout_rate']} | {row['learning_rate']} | {row['epoch']} | {row['metrics'].get('Validation/recall@20', '-')}"
                    f" | {row['metrics'].get('Validation/mrr@20', '-')} | {row['metrics'].get('Validation/loss', '-')} |\n")
        
        f.write("\nSorted by MRR@20 (highest to lowest, top 5):\n\n")
        f.write("| weight_decay | dropout_rate | learning_rate | epoch | MRR@20 | R@20 | Loss |\n")
        f.write("|-------------|--------------|---------------|-------|--------|------|------|\n")
        for row in sorted_by_mrr:
            f.write(f"| {row['weight_decay']} | {row['dropout_rate']} | {row['learning_rate']} | {row['epoch']} | {row['metrics'].get('Validation/mrr@20', '-')}"
                    f" | {row['metrics'].get('Validation/recall@20', '-')} | {row['metrics'].get('Validation/loss', '-')} |\n")
        ...
        f.write(f"  - Achieves R@20 = {best_recall['metrics'].get('Validation/recall@20', '-')} and MRR@20 = {best_recall['metrics'].get('Validation/mrr@20', '-')}\n")
        ...
        f.write(f"  - Achieves MRR@20 = {best_mrr['metrics'].get('Validation/mrr@20', '-')} and R@20 = {best_mrr['metrics'].get('Validation/recall@20', '-')}\n")
